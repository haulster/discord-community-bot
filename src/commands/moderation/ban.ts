...
const command: Command = {
  ...
  aliases: ["bannir"],
  category: "Modération",
  description: "Bannit un ou plusieurs membres du serveur.",
  ...
  run: async (_client, message, args) => {
    ...
    if (!authorAllowed) {
      await replyError(
        message,
        "Cette commande exige aussi la permission Discord **Bannir des membres**."
      );
      return;
    }
    ...
    if (targets.length === 0) {
      await replyError(message, `Syntaxe : \`${config.prefix}ban <membre...> [raison]\``);
      return;
    }
    if (targets.length > 5) {
    ...
    for (const input of targets) {
      ...
      const refusal = await checkSanctionTarget(message, member, "ban");
      if (refusal) {
        failures.push(`**${member.user.tag}** — ${refusal}`);
        continue;
      }

      // MP AVANT le bannissement (impossible après) — DMs fermés : ignoré proprement.
      await member
        .send({
          embeds: [
      ...
      const done = await member
        .ban({ reason: `${truncate(reason, 400)} — par ${message.author.tag}` })
        .then(() => true)
        .catch(() => false);
      if (done) successes.push(`**${member.user.tag}** (\`${member.id}\`)`);
      else failures.push(`**${member.user.tag}** — erreur Discord pendant le bannissement`);
    }

    // Résumé pour le modérateur.
    const summary = baseEmbed(config.colors.error)
      .setTitle(`⛔ Bannissement — ${successes.length} réussi(s), ${failures.length} échec(s)`)
      .addFields({ name: "Raison", value: truncate(reason, 1024), inline: false });
    if (successes.length > 0) {
      summary.addFields({ name: "Bannis", value: truncate(successes.join("\n"), 1024) });
    }
    ...
    // Log de modération.
    if (successes.length > 0) {
      const logEmbed = baseEmbed(config.colors.error)
        .setTitle("⛔ Bannissement(s)")
        .setDescription(
          [
